        ...
        if (has_presence($user_name)) {
            ...
            if ($_GET['username'] === "*") {
                ...
                if ($data > 1) {
                    ...
                    // User Not Found
                    $err = api_message("Oops!!", "User Not Found");
                    echo json_encode($err);
                }
            } else {
                $err =  api_message("Failed !", "You Need To Check The Documentation !");
                echo json_encode($err);
            }
        } else {
            $err =  api_message("Failed !", "You Need To Check The Documentation !");
...
if (is_put_request()) {
    ...
    if ($_GET['api_name'] == "edit_msg") {
        $text = $_GET['msg'] ?? "";
        $id = find_message_by_id(($_GET['id']) ?? "");
        $msg_id = $id['msg_id'] ?? "";
        if (!has_presence($text)) {
            $err =  api_message("Failed !", "You need to Provide A message");
            echo json_encode($err);
            exit;
        } elseif (!has_presence($msg_id)) {
            $err =  api_message("Failed !", "You Need to provide the id of the message ");
...
if (is_delete_request()) {
    ...
    if ($_GET['api_name'] == "delete_msg") {
        $id = $_GET['id'] ?? "";
        if (!has_presence($id)) {
            $err =  api_message("Failed !", "You need to Provide A Message ID");
            echo json_encode($err);
            exit;
        } else {
